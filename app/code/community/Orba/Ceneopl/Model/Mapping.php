<?php
class Orba_Ceneopl_Model_Mapping extends Mage_Core_Model_Abstract {
    
    protected function _construct() {
        $this->_init('ceneopl/mapping');
    } 
    ...
    public function saveCatalogCategories() {
        if ($this->getId()) {     
            $model = Mage::getModel('ceneopl/mapping_catalog_category');
    ...
    public function setCatalogCategoriesIds() {
        if ($this->getId()) {
            $collection = Mage::getModel('ceneopl/mapping_catalog_category')->getCollection()
                ->addFieldToFilter('mapping_id', $this->getId());
            $ids = array();
            foreach ($collection as $item) {
